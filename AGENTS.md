...
### Arcade v2 (juegos)
- Juegos con campo `dispositivo` ('ambos'/'pc'/'movil'): filtro en la página (`?dispositivo=pc|movil|ambos`). Un juego "ambos" aparece en PC y Celular.
- Actualizaciones de juegos publicados: el autor puede `POST /:id/actualizar` (máx. 1 pendiente a la vez, 409 si ya hay). El admin las revisa en la pestaña Juegos (`GET /admin/actualizaciones` solo pendientes, `POST /:id/revisar-actualizacion` con `decision` aprobar/rechazar + `motivo`). Al aprobar se aplican `version`/`novedades`/`dispositivo`/`url_recurso` y se vacía `actualizacion_pendiente`; al rechazar queda visible el motivo para el autor. Auditoría: `juego:actualizacion_aprobada`/`rechazada`.
- Historial de versiones (`juegos_versiones`, tabla auxiliar de `juegos`): el helper `registrarVersion()` (en `juegos.js`) escribe una fila al publicar un juego, al aprobar cambio de estado y al aprobar actualización. Endpoint público `GET /:id/versiones` (más reciente primero). En el Reproductor: sección "🔄 Historial de versiones" muestra la actual con etiqueta "· Actual" y toggle "▼ Ver versiones anteriores (N)" (CSS `.historial-version-*`).
- Las puntuaciones/tabla clasificadora se **descartaron** (no hay ruta ni tabla `juegos_puntuaciones`); el form de guardar puntaje y el ranking no existen en el Reproductor.
- Badge "🔄 Actualizado en vX.Y" en tarjetas (desplegable con novedades), panel "Mis juegos" del estudiante (estados + motivo rechazo + solicitar actualización), sección de novedades en el Reproductor.
- Versión siguiente auto-calculada sube la parte menor (1.1 → 1.2) en `ModalActualizarJuego`.
- Subida del juego como archivo `.html` (además de link de Scratch): bucket público `juegos`; `backend/src/config/almacenamiento.js` tiene `subirArchivoJuego(base64, nombre)` (valida extensión `.html`, ≤2 MB, content-type `text/html`) usado por `POST /api/archivos/subir-juego` (roles estudiante/admin/coordinador). `autor_grado` del juego se toma siempre de `beneficiarios` (por el documento del token), nunca del body.
- **Supabase sirve los `.html` públicos como `text/plain` (anti-XSS)** → la reproducción NO apunta al storage público: `GET /api/archivos/servir-juego?url=...` (proxy en `archivos.js`) valida que la URL sea del bucket `juegos` de este proyecto, descarga con service role y responde `text/html; charset=utf-8` quitando `X-Frame-Options` (para que el iframe del Reproductor funcione). El frontend convierte la URL con `urlReproduccionJuego()` en `frontend/src/config/api.ts`; aplicado en `ReproductorJuego.tsx` y en el preview de `TabJuegos.tsx`.
- Portada predeterminada: `frontend/public/placeholder-juego.svg` (fallback cuando el juego no tiene portada propia; reemplazó las URLs de Unsplash que se usaban antes).
